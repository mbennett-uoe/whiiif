...
""" index_with_plugin_scale.py: indexing tool for whiiif, for documents where the ALTO OCR is from a scaled version of the fullsize image """
...
def pipeline(alto, manifest, identifier, modify):
    ...
    scale = int(manifest_json["sequences"][0]["canvases"][0]["images"][0]["resource"]["height"]) / 1024
    dprint("SCALE CALCUATED: ", scale)
